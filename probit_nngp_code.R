...
for(mSub in c(15, 25, 50, 100)){
  ...
  t3 <- proc.time()
  spRnd_Grid <- Binary_prediction(lkVecPN_list[[which.max(lkVecPN)]], rbind(geomUnknownRnd, geomUnknownGrid), beta = 0, n.neighbors = n.neighbors, sigma.sq = alphaTLR[1], phi = alphaTLR[2], tau.sq = 0,
